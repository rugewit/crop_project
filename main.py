...
def initUI(self):
    ...
    self.btn_mode_2.setEnabled(False)
...
class MainWnd(QMainWindow):
    ...
    # окно для редактирования ячейки (на будущее)
    ...
    def start_croping(self):
        global img, img_path
        if settings.output_folder == None:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            temp_str1 = '<html><head/><body><p align="center"><span style=" font-size:12pt;">Ошибка</span></p></body></html>'
            temp_str2 = '<html><head/><body><p align="center"><span style=" font-size:12pt;">Выберете папку для разрезания</span></p></body></html>'
            msg.setText(temp_str1)
            msg.setInformativeText(temp_str2)
        ...
        for i in range(settings.count_x):
            for y in range(settings.count_y):
                ...
                crop(settings.output_folder, i * (settings.width // settings.count_x),
                     ...
                     x2, y2, i, y)
        temp_str = '<html><head/><body><p align="center"><span style=" font-size:16pt;">Картинка разрезана!</span></p></body></html>'
        QMessageBox().about(self, "Разрезание", temp_str)
